...
def is_chrome_debugging_running() -> bool:
    try:
        response = requests.get(f"http://127.0.0.1:{CONFIG['chrome_debug_port']}/json/version", timeout=3)
...
'''
    try:
        ...
        return False
        log("ERROR", f"调用 Claude Code 失败: {e}")
        return False
...
def start_gateway() -> bool:
    ...
    if is_gateway_running():
        ...
        if not is_chrome_debugging_running():
            log("INFO", "Gateway 启动成功，同步启动调试 Chrome...")
            start_chrome_debugging()
        else:
            log("INFO", "调试 Chrome 已在运行，跳过")
        return True
    else:
